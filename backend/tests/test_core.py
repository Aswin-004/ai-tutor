...
def _make_roadmap_json(n: int = 1, legacy_keys: bool = True) -> str:
    """Build a fake Gemini roadmap response.

    legacy_keys=True simulates an older-shaped response (module/topic) to
    exercise the backward-compat normalization path; legacy_keys=False
    simulates the current canonical (title/description) prompt contract.
    """
    title_key, desc_key = ("module", "topic") if legacy_keys else ("title", "description")
    return json.dumps([
        {
            title_key: f"Module {i}",
            desc_key: f"Topic {i}",
            "objectives": ["obj"],
            "time": "1 hour",
            "difficulty": "Beginner",
...
async def test_generate_roadmap_returns_list():
    ...
    assert result[0]["title"] == "Module 0"
    assert result[0]["step"] == 1
    assert result[4]["step"] == 5


@pytest.mark.asyncio
async def test_generate_roadmap_normalizes_legacy_module_topic_keys():
    ...
    assert result[0]["title"] == "Module 0"
    assert result[0]["description"] == "Topic 0"
...
# ── LearningSystem.generate_quiz ─────────────────────────────────────────────

def _make_quiz_json(correct_key: str = "correct_answer", include_explanation: bool = True) -> str:
    q = {
        "question": "What is recursion?",
        "options": ["A loop", "A function calling itself", "A variable", "A class"],
        correct_key: "A function calling itself",
    }
    if include_explanation:
        q["explanation"] = "Recursion is when a function calls itself to solve a smaller subproblem."
    return json.dumps([q])
...
async def test_generate_quiz_returns_correct_answer_and_explanation():
    ...
    assert quiz[0]["correct_answer"] == "A function calling itself"
    assert "Recursion" in quiz[0]["explanation"]


@pytest.mark.asyncio
async def test_generate_quiz_normalizes_legacy_answer_key():
    """A model response using the old 'answer' key should still score correctly."""
    profile = {"level": "Intermediate", "learning_style": "Visual", "subject": "DSA"}
    ...
    assert quiz[0]["correct_answer"] == "A function calling itself"
    assert quiz[0]["explanation"] == ""  # defaulted when the model omits it
...
async def test_generate_quiz_raises_on_ai_fallback():
    ...
    with patch("chromadb.PersistentClient"), \
        ...
        _patch_empty_vdb(system)
        with pytest.raises(RuntimeError, match="AI service unavailable"):
            await system.generate_quiz("recursion")
